...
same => n,Set(SEARCH_NUM=${IF($[${LEN(${CLEAN_NUM})} >= 8]?${CLEAN_NUM:-8}:${CLEAN_NUM})})
same => n,Set(CONTACT_NAME=${SHELL(sqlite3 /var/www/db/address_book.db "SELECT trim(coalesce(name, '')) || ' ' || trim(coalesce(last_name, '')) FROM contact WHERE (telefono IS NOT NULL AND telefono != '' AND (replace(replace(replace(telefono, ' ', ''), '-', ''), '+', '') LIKE '%${SEARCH_NUM}' OR '${CLEAN_NUM}' LIKE '%' || replace(replace(replace(telefono, ' ', ''), '-', ''), '+', ''))) OR (cell_phone IS NOT NULL AND cell_phone != '' AND (replace(replace(replace(cell_phone, ' ', ''), '-', ''), '+', '') LIKE '%${SEARCH_NUM}' OR '${CLEAN_NUM}' LIKE '%' || replace(replace(replace(cell_phone, ' ', ''), '-', ''), '+', ''))) OR (home_phone IS NOT NULL AND home_phone != '' AND (replace(replace(replace(home_phone, ' ', ''), '-', ''), '+', '') LIKE '%${SEARCH_NUM}' OR '${CLEAN_NUM}' LIKE '%' || replace(replace(replace(home_phone, ' ', ''), '-', ''), '+', ''))) LIMIT 1" | tr -d '\n' | tr -d '\r')})
